...
p = figure(width=1200, height=600,  x_axis_type='datetime', x_axis_label='Time', y_axis_label='Temperature', tools=[hover_tool])
...
async def update(x, y, y2):
    source1.stream(dict(x=[x], y=[y]))
...
async def main():
    ...
    def service_callback(service_event):
        ...
        if service_event.is_event and mode == 5:
            ...
            doc.add_next_tick_callback(partial(update, x=datetime.now(), y=int(service_event.data['value']) / 10, 
